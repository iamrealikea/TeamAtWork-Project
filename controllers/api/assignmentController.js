...
exports.getFileInUserAssignment = async (req, res) => {
    ...
    const teamId = req.params.tId;
    const assignId = req.params.aId;
    const team = await require('../../models/teamModel').getTeamById(teamId, sessionUserId);
    ...
    if (team.role === 'Manager') {
        files = await fileHandle.getAllFilesByAssignmentId(assignId);
    } else {
        files = await fileHandle.getFilesByAssignmentIdWithManager(assignId, sessionUserId, teamId);
    }
